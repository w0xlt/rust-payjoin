...
pub(crate) fn effective_socks_proxy_url(
    ...
    if let Some(session_socks_auth) = session_socks_auth {
        proxy.set_username(&session_socks_auth.username).map_err(|()| {
            anyhow::anyhow!("SOCKS proxy URL does not support embedding a username")
        })?;
        proxy.set_password(Some(&session_socks_auth.password)).map_err(|()| {
            anyhow::anyhow!("SOCKS proxy URL does not support embedding a password")
        })?;
    }
    Ok(proxy)
}
    ...
    fn test_config(socks_proxy: Option<url::Url>, tor_stream_isolation: bool) -> Config {
        Config {
            ...
            bitcoind: BitcoindConfig {
                rpchost: payjoin::Url::parse("http://127.0.0.1:18443")
