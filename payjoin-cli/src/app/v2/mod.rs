...
use payjoin::{ImplementationError, PjParam, Uri, Url};
...
where
    F: FnMut(&Url) -> std::result::Result<(payjoin::Request, C), E>,
    E: Into<anyhow::Error>,
{
    loop {
        ...
        match post_v2_request(config, req, session_socks_auth).await {
            ...
            Err(err) => {
                if is_socks_proxy_error(&err) {
                    tracing::error!(
                        "SOCKS proxy error while relaying through {relay} — failover skipped, check the proxy configuration: {err}"
                    );
                }
                return Err(err);
            }
    ...
    async fn send_payjoin(&self, bip21: &str, fee_rate: FeeRate) -> Result<()> {
        ...
        match uri.extras.pj_param() {
            ...
            PjParam::V2(pj_param) => {
                ...
                tokio::select! {
                    res = self.process_sender_session(sender_state, &persister, relay_manager) => return res,
                    _ = interrupt.changed() => {
...
impl App {
    ...
    async fn post_original_proposal(
        ...
        let endpoint = Url::parse(&sender.endpoint())?;
...
mod tests {
    ...
    fn is_socks_proxy_error_detects_tokio_socks_in_chain() {
        let direct: anyhow::Error = tokio_socks::Error::ProxyServerUnreachable.into();
        assert!(is_socks_proxy_error(&direct), "direct tokio_socks errors should be detected");

        let wrapped: anyhow::Error = anyhow::Error::new(tokio_socks::Error::PasswordAuthFailure(1))
            .context("connecting through SOCKS proxy");
        assert!(
            is_socks_proxy_error(&wrapped),
            "tokio_socks errors should be detected through anyhow context layers"
        );

        let unrelated: anyhow::Error = anyhow!("plain anyhow error with no SOCKS cause");
        assert!(
            !is_socks_proxy_error(&unrelated),
            "non-SOCKS errors should not be misclassified"
        );
    }

    #[test]
    fn should_fail_over_relay_skips_socks_proxy_errors() {
        let socks_err: anyhow::Error =
            anyhow::Error::new(tokio_socks::Error::ProxyServerUnreachable)
                .context("relay request through socks5h://127.0.0.1:9050 failed");
        assert!(
            !should_fail_over_relay(&socks_err),
            "SOCKS proxy errors must not trigger relay failover — every relay would fail the same way"
        );
    }
