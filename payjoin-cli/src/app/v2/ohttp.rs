...
fn select_relay(config: &Config, relay_manager: Arc<Mutex<RelayManager>>) -> Result<Url> {
    ...
    loop {
        ...
        match ohttp_keys {
            ...
            Err(err) => {
                ...
                if super::is_socks_proxy_error(&err) {
                    tracing::error!(
                        "SOCKS proxy error while bootstrapping through {selected_relay} — failover skipped, check the proxy configuration: {err:?}"
                    );
                    return Err(err);
                }
