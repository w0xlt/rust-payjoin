...
mod e2e {
    ...
    use crate::e2e_helpers::{
        assert_socks_proxy_usage, get_bip21_from_receiver, terminate, V2NetworkConfig, RECEIVE_SATS,
    };
    #[cfg(not(feature = "v2"))]
    use crate::e2e_helpers::{terminate, RECEIVE_SATS};
    ...
    async fn send_receive_payjoin_v2_to_v1() -> Result<(), Box<dyn std::error::Error + Send + Sync>>
    {
        ...
        async fn send_v2_receive_v1_async(
            ...
            let payjoin_sent = tokio::time::timeout(timeout, rx.recv())
                ...
                .expect("rx channel closed prematurely");
