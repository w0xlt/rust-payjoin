...
use crate::ohttp_relay::{empty, GatewayConnector, GatewayUri};
    ...
    B: Send + Debug + 'static,
{
    tokio::task::spawn(async move {
        match hyper::upgrade::on(req).await {
            Ok(upgraded) => {
                if let Err(e) = tunnel(upgraded, gateway_origin, connector).await {
...
async fn tunnel(
    ...
    let mut server = connector.connect(&gateway_origin).await?;
