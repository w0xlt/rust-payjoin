...
impl Debug for SocksProxyConfig {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        f.debug_struct("SocksProxyConfig").field("url", &url).finish()
    }
}
...
impl SocksProxyConfig {
    ...
    async fn connect_to(&self, host: &str, port: u16) -> std::io::Result<tokio::net::TcpStream> {
        let target_host = normalize_socks_host(host);
        let proxy_host =
            self.url.host_str().ok_or_else(|| std::io::Error::other("missing SOCKS proxy host"))?;
        let proxy_host = normalize_socks_host(proxy_host);
        let proxy_port = self
            .url
            .port_or_known_default()
            .ok_or_else(|| std::io::Error::other("missing SOCKS proxy port"))?;

        let stream = if let Some((username, password)) = socks_url_auth(&self.url) {
            ...
            tokio_socks::tcp::Socks5Stream::connect((proxy_host, proxy_port), (target_host, port))
                .await
                .map_err(std::io::Error::other)?
        };
...
fn normalize_socks_host(host: &str) -> &str {
    let Some(unbracketed) = host.strip_prefix('[').and_then(|host| host.strip_suffix(']')) else {
        return host;
    };
...
mod tests {
    ...
    #[test]
    fn relay_config_debug_redacts_socks_proxy_credentials() {
        let socks_proxy = SocksProxyConfig::parse(Some(
            Url::parse("socks5h://proxyuser:super-secret@127.0.0.1:9050")
    ...
    #[test]
    fn socks_target_host_strips_ipv6_uri_brackets() {
        let gateway = GatewayUri::from_str("http://[::1]:8080").expect("IPv6 gateway should parse");

        assert_eq!(gateway.host(), "[::1]");
        assert_eq!(normalize_socks_host(gateway.host()), "::1");
    }

    #[test]
    fn socks_proxy_host_strips_ipv6_uri_brackets() {
        let proxy = Url::parse("socks5h://[::1]:9050").expect("IPv6 proxy should parse");

        assert_eq!(proxy.host_str(), Some("[::1]"));
        assert_eq!(normalize_socks_host(proxy.host_str().expect("proxy host should exist")), "::1");
    }

    #[test]
    fn socks_target_host_preserves_non_ipv6_hosts() {
        assert_eq!(normalize_socks_host("directory.example"), "directory.example");
        assert_eq!(normalize_socks_host("127.0.0.1"), "127.0.0.1");
        assert_eq!(normalize_socks_host("[directory.example]"), "[directory.example]");
    }
