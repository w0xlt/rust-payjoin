    ...
    fn call(&mut self, dst: Uri) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            let port = dst
                ...
                .ok_or_else(|| {
                    std::io::Error::other("destination URI is missing a known port")
                })?;
...
mod tests {
    ...
    async fn parse_gateway_uri_accepts_https_gateway_in_socks_mode() {
        ...
        let https_gateway = GatewayUri::from_str("https://directory.example")
            .expect("HTTPS gateway should parse");
        config
            .prober
            .assert_opt_in(&https_gateway)
            .await
            .expect("asserting opt in should succeed");

        let gateway = parse_gateway_uri(
            &Method::GET,
    ...
    async fn parse_gateway_uri_accepts_connect_gateway_in_socks_mode() {
        ...
        let authority: Authority = "directory.example:443"
            .parse()
    ...
    async fn forward_request_streams_gateway_response_body() {
        let gateway_listener = TcpListener::bind("127.0.0.1:0")
            .await
            .expect("streaming test gateway should bind");
    ...
    async fn socks_client_uses_manual_root_store_for_https_gateway() {
        ...
        let tls_listener = TcpListener::bind("127.0.0.1:0")
            .await
            .expect("TLS test gateway should bind");
        let tls_port = tls_listener
            .local_addr()
            .expect("TLS test gateway should have a local address")
            .port();
        let tls_acceptor = {
        ...
        let gateway_handle = tokio::spawn(async move {
            loop {
                let (stream, _) = tls_listener
                    .accept()
                    .await
                    .expect("TLS test gateway should accept a client");
                ...
                tokio::spawn(async move {
                    ...
                    stream
                        .write_all(body)
                        .await
                        .expect("TLS test gateway should write HTTP body");
                    stream
                        .shutdown()
                        .await
                        .expect("TLS test gateway should close the TLS stream");
                });
            }
        });
        ...
        let socks_handle = tokio::spawn(async move {
            loop {
                let (stream, _) = socks_listener
                    .accept()
                    .await
                    .expect("SOCKS test proxy should accept a client");
                tokio::spawn(async move {
